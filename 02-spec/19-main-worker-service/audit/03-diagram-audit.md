...
**Spec audited:** `02-spec/19-main-worker-service/diagrams/` + `images/`
...
| ID | Finding | Severity |
|----|---------|----------|
| F-D-09 | The schema in this ERD differs from the canonical `02-spec/05-split-db-architecture/`. This file is a **projection** but the diagram does not say "non-authoritative — see spec/05". A dumb AI will treat it as truth and diverge from `spec/05`. | BLOCKER |
...
2. **F-D-09** — `erd-worker-split-db.mmd` claims to be a projection but is missing a banner saying so and contradicts `02-spec/05-split-db-architecture/` in detail.
...
| Step | File | Focus |
|------|------|-------|
| 4 | `04-cross-spec-dependency-audit.md` | Verify external `02-spec/03/04/05/06/14` anchors actually exist |
...
**Method:** For each F-D finding, look up its closing task in `.ai-memory/question-and-ambiguity/task-counter.md` and confirm by inspecting the diagram or its authority anchor.
...
| F-D-22 | MAJOR | ✅ CLOSED | #14+#15 | Auth header conventions in `02-spec/04/06-rest-api-format.md`; banner defers |
...
| F-D-32 | BLOCKER | ✅ CLOSED | #07+#15 | `seq-push-update.mmd` cites `PublishZip` + JID flow per `02-spec/14-update/28-worker-push-instruction.md` |
