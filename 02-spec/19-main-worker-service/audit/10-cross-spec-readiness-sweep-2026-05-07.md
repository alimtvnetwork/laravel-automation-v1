...
> **Scope:** All `02-spec/` folders **except** `02-spec/19-main-worker-service/` (which is closed at 96/100 by audit-09).
...
### 2.1 `02-spec/03-error-manage/02-error-architecture/07-logging-and-diagnostics/02-session-based-logging.md`
...
### 2.2 `02-spec/14-update/27-generic-installer-behavior.md`
...
### 2.3 `02-spec/14-update/23-install-script-version-probe.md`
...
### 2.4 `02-spec/15-distribution-and-runner/06-fix-repo-forwarding.md`
...
`02-spec/06-seedable-config-architecture/02-features/04-rag-test-coverage-matrix.md:251` — Mutation Score target shown as `TBD`. Either fix a number or strike the row; non-blocking.
...
- **No stale audits** outside `02-spec/19-…/audit/` (which are already STALE-banner'd).
