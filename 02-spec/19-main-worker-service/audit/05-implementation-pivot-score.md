...
| 4 | **Register worker error codes** in `02-spec/03-error-manage/03-error-code-registry/` | F-X-08, F-A-21, F-B-08 | AC-6, AC-1 |
| 5 | **Author "JSON instruction document" format** under `02-spec/14-update/` (new sub-doc, e.g. `28-worker-push-instruction.md`) | F-X-14, F-X-15, F-X-17 | AC-9 |
| 6 | **Define `EnumPage` + `RolePageAccess` seed** in `02-spec/06-` and reference from spec/19 | F-B-09, F-B-10, F-X-06 | AC-5 |
| 7 | **Pin retry caps + idempotency-key TTL** to single values; remove the 3-vs-5 contradiction | F-A-15, F-A-16, F-B-12 | AC-7, AC-6 |
| 8 | **Promote header conventions** (`X-Correlation-Id`, `X-Idempotency-Key`, `X-Auth-Action`) into `02-spec/04-database-conventions/06-rest-api-format.md` | F-X-10, F-A-22 | AC-7, AC-4 |
...
| 5 | JSON instruction document format | #07 (`02-spec/14-update/28-worker-push-instruction.md`) | ✅ |
...
| 8 | Promote header conventions to spec/04 | #14 (`02-spec/04/06-rest-api-format.md`) | ✅ |
...
| JSON schema of worker push-update instruction | ❌ | ✅ #07 `02-spec/14-update/28-` |
| Header-name list sourced from spec/04 | ❌ | ✅ #14 `02-spec/04/06-rest-api-format.md` |
