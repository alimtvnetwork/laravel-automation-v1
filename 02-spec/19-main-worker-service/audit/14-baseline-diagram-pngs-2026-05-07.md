...
- **Failed (parser errors, pre-existing — not introduced here):**
  - `02-spec/12-cicd-pipeline-workflows/images/ci-pipeline-flow.mmd` — line 3 `actions/checkout@v6` token rejected by mermaid v11 graph parser.
  - `02-spec/19-main-worker-service/diagrams/seq-incremental-backup.mmd` — line 27 sequence-diagram `Note over` placement after `alt` block rejected.
